...
namespace MDUA.DataAccess
{
	public partial class ProductDataAccess : BaseDataAccess, IProductDataAccess
	{
		...
		{	
			...
			AddParameter(cmd, pDecimal(ProductBase.Property_PriceMultiplier, 5, productObject.PriceMultiplier));
		}
		#endregion
		...
		protected void FillObject(ProductBase productObject, SqlDataReader reader, int start)
		{
				...
				productObject.ProductName = reader.GetString( start + 1 );			
				productObject.ReorderLevel = reader.GetInt32( start + 2 );			
				productObject.CompanyId = reader.GetInt32( start + 3 );			
				productObject.PriceMultiplier = reader.GetDecimal( start + 4 );			
			FillBaseObject(productObject, reader, (start + 5));
