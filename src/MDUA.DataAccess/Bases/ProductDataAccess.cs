...
{
    // This is now a SINGLE class that inherits from BaseDataAccess
    // and implements IProductDataAccess
    public class ProductDataAccess : BaseDataAccess, IProductDataAccess
    {
        ...
        private void AddCommonParams(SqlCommand cmd, ProductBase productObject)
        {
            ...
            AddParameter(cmd, pNVarChar(ProductBase.Property_Description, -1, productObject.Description)); // -1 for nvarchar(max)
            AddParameter(cmd, pNVarChar(ProductBase.Property_Slug, 400, productObject.Slug));
            AddParameter(cmd, pDecimal(ProductBase.Property_BasePrice, productObject.BasePrice));
        ...
        public ProductDetailsModel GetProductDetails(int productId)
        {
            ...
            using (SqlCommand cmd = GetSPCommand("sp_GetProductDetails"))
            {
                ...
                using (reader)
                {
                    ...
                    if (reader.Read())
                    {
                        model.Product = FillProduct(reader); // Uses our private helper
                    }
                    else
                    {
                        return model; // Product not found
                    }
        ...
        {
            var p = new Product();
            FillObject(p, reader); // Reuses the protected method from this class
            return p;
        }
