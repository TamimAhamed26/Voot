...
namespace MDUA.DataAccess
{
	public partial class AddressDataAccess : BaseDataAccess, IAddressDataAccess
	{
		...
		protected void FillObject(AddressBase addressObject, SqlDataReader reader, int start)
		{
				...
				addressObject.ZipCode = reader.GetString( start + 6 ).ToCharArray();			
				if(!reader.IsDBNull(7)) addressObject.Country = reader.GetString( start + 7 );			
				if(!reader.IsDBNull(8)) addressObject.AddressType = reader.GetString( start + 8 );			
				addressObject.CreatedBy = reader.GetString( start + 9 );			
				addressObject.CreatedAt = reader.GetDateTime( start + 10 );			
				if(!reader.IsDBNull(11)) addressObject.UpdatedBy = reader.GetString( start + 11 );			
				if(!reader.IsDBNull(12)) addressObject.UpdatedAt = reader.GetDateTime( start + 12 );			
			FillBaseObject(addressObject, reader, (start + 13));
